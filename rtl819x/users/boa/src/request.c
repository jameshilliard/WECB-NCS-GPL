...
#ifdef  AEI_ZERO_CONF
char after_login_last_url[64] = {0};
#endif
#endif
	...
	{		
		...
		{				
			if ((last_request_uri = (char *) malloc(strlen(req->request_uri) + 1)) != NULL)
			{
				...
				//fprintf(stdout,"\ngive to last  page=%s^-^\n",req->request_uri);
				if(!strstr(req->request_uri, "forgot_password.html")&&!strstr(req->request_uri, "wcb_thankyou.html")&&!strstr(req->request_uri, "wirelesschanging_notify.html"))
			...
			if(!strstr(req->request_uri, "forgot_password.html")&&!strstr(req->request_uri, "wcb_thankyou.html"))
#endif
...
#ifdef  AEI_ZERO_CONF
        			if(!strstr(req->request_uri, "forgot_password.html")&&!strstr(req->request_uri, "wcb_thankyou.html")&&!strstr(req->request_uri, "wirelesschanging_notify.html")&&!strstr(req->request_uri, "wcb_countdown.html"))
        			{
        				 memset(after_login_last_url, 0, strlen(req->request_uri) + 1);        				
        			 	 strncpy(after_login_last_url, req->request_uri, strlen(req->request_uri)); 
        			}
#endif					
			}
		}	
