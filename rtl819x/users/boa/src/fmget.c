...
#endif
#ifdef  AEI_ZERO_CONF
extern char after_login_last_url[64];
extern char *last_request_uri;
#endif

int telus_langstat = 0;
int have_default = 0;
...
int getInfo(request *wp, int argc, char **argv)
{
	...
	else if(!strcmp(name, "localsshparams"))
	{
		buffer[0]='\0';
		actgetSshParams(buffer);	
		return req_format_write(wp, "%s", buffer);
 	}
#ifdef ACTIONTEC_CLIENT
...
#ifdef  AEI_ZERO_CONF
	else if(!strcmp(name, "last_req_url"))
	{
                if(strstr(after_login_last_url,".html"))
			return req_format_write(wp, after_login_last_url);
		else
			return req_format_write(wp, last_request_uri);
	}
#endif	
	else if(!strcmp(name, "telus_langstat"))
	{
		if(have_default == 0)
		{
#ifdef ACTIONTEC_MULTIACCESS
		...
		{
			return req_format_write(wp, "%d",telus_langstat);
		}
		else if(2==lang_auth_level)
		{
			if(tr69GetUnfreshLeafData(buffer, "Device.Users.User.2", "Language"))	
	        	{
				kerror("error_buf: Get Device.Users.User.2.Language error!");
	        	}
			return req_format_write(wp, "%s",buffer);
		}
		else if(3==lang_auth_level)
		{
			if(tr69GetUnfreshLeafData(buffer, "Device.Users.User.3", "Language"))	
	        	{
